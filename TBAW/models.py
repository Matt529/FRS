...
class Team(models.Model):
    ...
    locality = models.CharField()  # e.g. city
    region = models.CharField()  # e.g. state, province
    country_name = models.CharField()
    location = models.CharField()  # full city + state + country

    team_number = models.PositiveIntegerField()
    key = models.CharField(max_length=8)  # e.g. frc2791
    nickname = models.CharField()  # shorter name
...
class Event(models.Model):
    ...
    name = models.CharField()  # e.g. Finger Lakes Regional
    short_name = models.CharField(null=True)  # e.g. Finger Lakes
    event_code = models.CharField(max_length=6)  # e.g. cmp
...
class Match(models.Model):
    key = models.CharField()  # yyyy{EVENT_CODE}_{COMP_LEVEL}m{MATCH_NUMBER}
    comp_level = models.CharField()  # e.g. qm, ef, qf, sf, f

    """The set number in a series of matches where more than one match is required in the match series.
    2010sc_qf1m2, would be match 2 in quarter finals 1.
    -The Blue Alliance"""
    set_number = models.CharField(null=True)

    match_number = models.CharField()  # e.g. 2016nyro_qm20
